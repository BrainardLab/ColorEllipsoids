...
for s = 1:nDataFiles
    ...
    load(['Fits_isothreshold_',plane_slc,'_sim',num2str(numSimPerCond),...
        'perCond_samplingNearContour_jitter',num2str(samplingJitter),...
...
for i = 1:length(sim.slc_ref)
    for j = 1:length(sim.slc_ref)
        ...
        %full model
        axisRatio_fullM = squeeze((1/modelComp.fullM_ellParam(i,j,1,:))./(1/modelComp.fullM_ellParam(i,j,2,:)));
        axisRatio_fullM_sorted = sort(axisRatio_fullM, 'ascend');
        modelComp.axisRatio_fullM_mean(i,j) = mean(axisRatio_fullM);
        modelComp.axisRatio_fullM_lb(i,j) = axisRatio_fullM_sorted(errbar_lb_idx);
        modelComp.axisRatio_fullM_ub(i,j) = axisRatio_fullM_sorted(errbar_ub_idx);

        rotAngle_fullM = deg2rad(squeeze(modelComp.fullM_ellParam(i,j,3,:)));
        rotAngle_fullM_sorted = sort(rotAngle_fullM, 'ascend');
        modelComp.rotAngle_fullM_mean(i,j) = mean(rotAngle_fullM_sorted);
        modelComp.rotAngle_fullM_lb(i,j) = rotAngle_fullM_sorted(errbar_lb_idx);
        modelComp.rotAngle_fullM_ub(i,j) = rotAngle_fullM_sorted(errbar_ub_idx);  

        %contour CI
        [modelComp.unscaled_contour_CI_ub_fullM(i,j,:,:), ...
            modelComp.unscaled_contour_CI_lb_fullM(i,j,:,:),...
            modelComp.contour_CI_ub_fullM(i,j,:,:), modelComp.contour_CI_lb_fullM(i,j,:,:)] = ...
            compute_contour_CI(squeeze(modelComp.fullM_fitEllipse_sampled_comp_unscaled(i,j,:,:,:)),...
...
plot_2D_isothreshold_contour(stim.x_grid_ref, stim.y_grid_ref, ...
    ...
    'IndividualEllipses_contour_CI',{modelComp.contour_CI_lb_fullM, modelComp.contour_CI_ub_fullM},...
    ...
    'subTitle', {sprintf(['Comparison of the predictions for the iso-threshold contours \nin ',...
        plane_slc, ' between the Wishart model and the full model'])},...
    'figName', ['Comparison_IsothresholdContours_WishartM_vs_fullM_',plane_slc,'_sim',num2str(sim.nSims),'perCond_',...
...
figName1 = ['Comparison_axisRatio_WishartM_vs_fullM_',plane_slc,'_sim',num2str(sim.nSims),'perCond_',...
            'sampling',sim.method_sampling,'_jitter',num2str(sim.random_jitter),'_rng1to',num2str(nDataFiles)];
plot_comparison_WishartM_vs_fullM(modelComp.axisRatio_fullM_mean,...
    modelComp.axisRatio_WishartM_mean,modelComp.axisRatio_WishartM_lb,...
    modelComp.axisRatio_WishartM_ub,modelComp.axisRatio_fullM_lb,...
    modelComp.axisRatio_fullM_ub,...
    'xlim',[1,7],'ylim',[1,7],'x_ref',stim.grid_ref,'y_ref',stim.grid_ref,...
    'xlabel',sprintf('The ratio of major vs. minor axis\n computed based on the Individual Ellipses Model'),...
...
%rotation angle
figName2 = ['Comparison_rotAngle_WishartM_vs_fullM_',plane_slc,'_sim',num2str(sim.nSims),'perCond_',...
            'sampling',sim.method_sampling,'_jitter',num2str(sim.random_jitter),'_rng1to',num2str(nDataFiles)];
plot_comparison_WishartM_vs_fullM(modelComp.rotAngle_fullM_mean,...
    modelComp.rotAngle_WishartM_mean,modelComp.rotAngle_WishartM_lb,...
    modelComp.rotAngle_WishartM_ub,modelComp.rotAngle_fullM_lb,...
    modelComp.rotAngle_fullM_ub,...
    ...
    'xlabel',sprintf('The rotation (in radians)\n computed based on the Individual Ellipses Model'),...
...
if strcmp(sim.method_sampling, 'NearContour')
    fileName = ['Comparison_WishartM_vs_fullM_',plane_slc,'_sim',...
