...
plt.nPhiEllipse        = 100;
plt.circleIn3D         = UnitCircleGenerate_3D(plt.nThetaEllipse, plt.nPhiEllipse);

%% Fitting starts from here
%indices for the varying chromatic directions 
%GB plane: [2,3]; RB plane: [1,3]; RG plane: [1,2]; all directions: [1,2,3]
idx_varyingDim = 1:3;
    ...
    for j = 1:stim.nGridPts_ref
        for k = 1:stim.nGridPts_ref
            ...
            [results.fitEllipsoid(i,j,k,:,:), ...
                ...
                fit_3d_isothreshold_contour(rgb_ref_ijk, [],stim.grid_xyz, ...
...
plot_3D_isothreshold_ellipsoid(stim.grid_ref, stim.grid_ref, ...
    stim.grid_ref,results.fitEllipsoid, plt.nThetaEllipse,...
    plt.nPhiEllipse,'slc_x_grid_ref',1:2:5, 'slc_y_grid_ref',...
    1:2:5, 'slc_z_grid_ref',1:2:5, 'saveFig',false);
