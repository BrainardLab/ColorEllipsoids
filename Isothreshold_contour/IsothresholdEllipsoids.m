...
flag_visualize_chromDir = true;
slice_slc = [3, 5, 7];
if flag_visualize_chromDir
    figure; colormap(white)
    surf(horzcat(stim.grid_x, stim.grid_x(:,1)), ...
        horzcat(stim.grid_y,stim.grid_y(:,1)),...
        horzcat(stim.grid_z, stim.grid_z(:,1)),'FaceAlpha',0); hold on
    % for s = 1:length(slice_slc)
    %     ss = slice_slc(s);
    % 
    %     % Define the vertices of the 2D plane in 3D space
    %     X_plane = [stim.grid_x(ss,:) stim.grid_x(ss,1)];
    %     Y_plane = [stim.grid_y(ss,:) stim.grid_y(ss,1)];
    %     Z_plane = stim.grid_z(ss,1) * ones(size(X_plane));
    % 
    %     % Plot the surface using the defined vertices
    %     fill3(X_plane, Y_plane, Z_plane, [0.85,0.85,0.85],...
...
if load_existingFits
    ...
    load Isothreshold_ellipsoid_CIELABderived.mat
    results = D{3}; 
else
