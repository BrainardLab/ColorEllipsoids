...
par_all = jnp.array([[1.5,  0.5, 0.2, 0.7, 0.1],  # Params for ellipse 1
                     [  1,    1,  1,  2, 1],  # Params for ellipse 2 (circle)
                     [1, 0.5, 1.2, 1.6, 0.5],  # Params for ellipse 3
                     [2.5,    2, 1, 0.3, 0.5]]) # Params for ellipse 4
...
# Create 3D subplots
fig2 = plt.figure(figsize=(12, 3))  # Adjust size for 4 subplots

# Plot the transformed points in 3D
for i in range(4):
    ax = fig2.add_subplot(1, 4, i+1, projection='3d')  # Create a 3D subplot for each ellipse
...
xyz_recover = np.full(xyz_comp.shape, np.nan)  # Initialize array to store recovered ellipses
for i in range(nRefs):
    eigvec_i = rotation_angles_to_eigenvectors(*params_recover[i,3:5])
    ...
    xyz_recover_i = PointsOnEllipsoid(params_recover[i,0:3],  # Semi-major and semi-minor axes
                                      np.reshape(xyz_ref[i],[3,1]),
                                      eigvec_i, unitEll)       # Center of ellipse, number of points
    
    xyz_recover[i] = np.transpose(xyz_recover_i,[1,0])
    
# Final visualization: compare recovered ellipses with ground truth ellipses
fig = plt.figure(figsize=(12, 3))  # Adjust size for 4 subplots
for i in range(4):
    ax = fig.add_subplot(1, 4, i+1, projection='3d')  # Create a 3D subplot for each ellipse
    # Plot original (ground truth) ellipses in red
    ax.plot(xyz_comp[i,:,0], xyz_comp[i,:,1], xyz_comp[i,:,2], c='r', lw=3)
    
    # Scatter plot of jittered comparison points
    ax.scatter(xyz_jitter[i,:,0], xyz_jitter[i,:,1], 
                  xyz_jitter[i,:,2], s=2, c='gray', alpha=0.1)
    ...
    ax.scatter(*xyz_ref[i], c='r', marker='+')
