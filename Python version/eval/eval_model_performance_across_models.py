...
nSims     = [320, 240, 160, 80]     # Number of simulations per reference
nRefs     = 25
...
path_str1 = base_dir + 'ELPS_analysis/ModelFitting_DataFiles/2D_oddity_task_indvEll/' 
...
for k in nSims:        
    ...
    for l in seed_list:            
        ...
        file_name_jj = f'Fitted_isothreshold_{plane_2D}_sim{k}perCond_samplingNearContour_' +\
...
for l in seed_list:
    # Construct the file name based on the jitter level and other parameters
    file_name_jj = f'Fitted_isothreshold_{plane_2D}_sim{nSims_Wishart}perCond_'+\
...
BW_distance_output_indvEll = np.full((nLevels, len(seed_list), 5, 5), np.nan)
...
BW_distance_output_median_indvEll = np.median(BW_distance_output_indvEll, axis = (1,2,3))
...
yerr_indvEll = np.array([BW_distance_output_median_indvEll - \
                            np.min(BW_distance_output_indvEll, axis = (1,2,3)), 
                 np.max(BW_distance_output_indvEll, axis = (1,2,3)) - \
                     BW_distance_output_median_indvEll])
    
BW_distance_output_median_Wishart = np.median(BW_distance_output_Wishart, axis = (0,1,2))
# Create the error for yerr as a tuple of arrays for asymmetrical error
yerr_Wishart = np.array([BW_distance_output_median_Wishart - \
                            np.min(BW_distance_output_Wishart, axis = (0,1,2)), 
                 np.max(BW_distance_output_Wishart, axis = (0,1,2)) - \
