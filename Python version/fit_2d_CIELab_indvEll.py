...
model_indv = IndividualProbSurfaceModel(NUM_GRID_PTS, 
                                        [1e-2,0.3],  # Bounds for radii
                                        [jnp.pi/5, 2*jnp.pi],  # Bounds for angle in radians
...
for k in KEY_list:    
    ...
    init_params = model_indv.sample_params_prior(KEY_k)  # Sample initial parameters for the model
    ...
    params_recover_k, iters, objhist_k = optimize_nloglikelihood(
        ...
        bds_radii = model_indv.bds_radii,
        bds_angle = model_indv.bds_angle,
        learning_rate = 1e-1,
...
class model_pred:
    def __init__(self, fitEll_unscaled, target_pC = 2/3):
        self.fitEll_unscaled = fitEll_unscaled
        self.target_pC = target_pC
model_pred_indvEll = model_pred(np.reshape(xy_recover,(NUM_GRID_PTS, NUM_GRID_PTS,2, nTheta)))
