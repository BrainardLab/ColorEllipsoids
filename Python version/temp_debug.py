...
BANDWIDTH    = 1e-5        # Bandwidth for logistic density function.
...
#%%
file_name3 = 'Fitted_isothreshold_'+plane_2D+'_sim240perCond_samplingNearContour_jitter0.3_bandwidth1e-05.pkl'
...
def convert_Sig_2DisothresholdContour_oddity(rgb_ref, varying_RGBplane,
    ...
    Uref = 0.1*model.compute_U(W, rgb_ref_s)
    U0 = 0.1*model.compute_U(W, rgb_ref_s)
    ...
    for i in range(numDirPts):
        ...
        for x in range(nSteps_bruteforce):
            ...
            U1 = 0.1*model.compute_U(W, rgb_comp)
            if x == nSteps_bruteforce -1:
                print(x)
            ...
            pChoosingX1[x] = 1.0 - oddity_task.approx_cdf_one_trial(\
...
for i in range(NUM_GRID_PTS):
    ...
    for j in range(NUM_GRID_PTS):
        ...
        vecLength_test = np.linspace(min(vecLength_ij)*0.5, max(vecLength_ij)*20,\
            ngrid_bruteforce)
...
for i in range(NUM_GRID_PTS):
    for j in range(NUM_GRID_PTS):
        ...
        viz.plot_ellipse(
            ax, xgrid[i, j,:], 0.1* Sigmas_est_grid[i, j,:,:], color="k", alpha=.5, lw=1
       )
