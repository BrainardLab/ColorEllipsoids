...
for i in range(ref_size_dim1*ref_size_dim2):
    ...
    ax[2].scatter(x1_jnp[i_lb:i_ub,0] - xref_jnp[i_lb:i_ub,0],\
                ...
                c = cm, s = 2, alpha = 0.5)
    if i in [0,1]:
        ax[i].set_xticks(np.unique(xref_jnp))
        ax[i].set_yticks(np.unique(xref_jnp))
...
for i in range(NUM_GRID_PTS):
    ...
    for j in range(NUM_GRID_PTS):
        ...
        vecLength_test = np.linspace(min(vecLength_ij)*0.5, max(vecLength_ij)*2.5,\
            ngrid_bruteforce)
        ...
        recover_fitEllipse_scaled[i,j,:,:], recover_fitEllipse_unscaled[i,j,:,:],\
            ...
            recover_rgb_comp_est[i,j,:,:], _ = \
                convert_Sig_2DisothresholdContour_oddity(\
                    ...
                    stim['grid_theta_xy'], vecLength_test, sim['pC_given_alpha_beta'],\
                    W_est)
